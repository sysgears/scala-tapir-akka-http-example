...
class AuthController(authService: ULayer[Authentication]) {
  ...
  val signInEndpoint = endpoint.post // POST endpoint
    ...
    .in(
      jsonBody[SignInForm] // requires signInForm in request body, added description and example.
        .description("Required data to log in")
        .example(SignInForm("test@example.com", "pass4567"))
    )
    .out(
      jsonBody[Token]
  ...
  val signUpEndpoint = endpoint.post // POST endpoint
    ...
    .in(
      jsonBody[SignUpForm] // requires signUpForm in request body, added description and example.
    ...
    .out(
      statusCode(StatusCode.Created)
        .description("Returns Created when user is registered")
    ) // defining static status code for success response.
    .errorOut(
      oneOf[ErrorInfo](
        oneOfVariant(
          statusCode(StatusCode.Conflict)
            .and(jsonBody[Conflict].description("When something not found"))
        ),
        oneOfVariant(
          statusCode(StatusCode.BadRequest).and(
            jsonBody[BadRequest].description(
              "if body is incorrect, if passwords not matches or password is too short"
            )
          )
        ),
        oneOfDefaultVariant(
          jsonBody[com.example.errors.ErrorMessage]
            .description("Default result")
            .example(com.example.errors.ErrorMessage("Test error message"))
        )
      )
    )
    .serverLogic { signUpForm => // defined logic for the endpoint.
      ZioUtil.foldRunToFuture(
        AuthService.signUp(signUpForm).provide(authService)
      )
    }

  /** Convenient way to assemble endpoints from the controller and then concat this route to main route. */
  val authRoutes: List[ServerEndpoint[AkkaStreams with WebSockets, Future]] =
    List(signInEndpoint, signUpEndpoint)
}
