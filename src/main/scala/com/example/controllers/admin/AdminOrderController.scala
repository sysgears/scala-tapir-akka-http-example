...
class AdminOrderController(tapirSecurity: TapirSecurity,
                           adminOrderService: ULayer[AdminOrders]) {

  /**
    * Retrieves paginated orders
    */
  val adminOrdersView = tapirSecurity
    .tapirSecurityEndpoint(List(Roles.Admin)) // restricted, admins only
    .get // GET endpoint
    .description(
      "Showing paginated orders for admin with opportunity to sort by some parameters"
    ) // endpoint description
    .in("admin" / "orders") // /admin/orders uri
    .in(EndpointInput.derived[PaginatedEndpointArguments]) // args defined in that class
    .out(
      jsonBody[AdminOrderViewResponse].description(
        "Paginated list of orders, zipped with user, who made this order"
      )
    ) // defined response
  ...
  val changeOrderStatusEndpoint = tapirSecurity
    .tapirSecurityEndpoint(List(Roles.Admin))
    .put // PUT endpoint
    .description(
      "Updates order's status. Can change to unprocessed, processed or complete"
    ) // endpoint description
    ...
    .serverLogic { _ => args => // server logic
      if (Order.appropriateStatuses.contains(args.newStatus.toLowerCase())) { // new status validation
        ZioUtil.foldRunToFuture(
          AdminOrderService.updateOrderStatus(args).provide(adminOrderService)
        )
      } else {
