...
class OrderController(tapirSecurity: TapirSecurity,
                      orderService: ULayer[OrdersService]) {

  /**
    * Create order endpoint.
    */
  val createOrderEndpoint = tapirSecurity
    .tapirSecurityEndpoint(List(Roles.User)) // accessible only for users with role User
    .post // POST endpoint
    .in("orders") // /orders uri
    .description("Creates order for the user")
    .in(
      jsonBody[CreateOrderForm] // request has to have body of CreateOrderForm
    ...
    .serverLogic { user => newOrder => // security output => endpoint input => server logic
      if (newOrder.products.forall(
            product => product.quantity > 0 && product.productId.nonEmpty
          )) {
        ZioUtil.foldRunToFuture(
          OrderService.createOrder(user.id, newOrder).provide(orderService)
        )
      } else {
        Future.successful(
          Left(BadRequest("Some order record contains invalid value!"))
        )
    }
    }

  /** get user's orders list view endpoint definition. */
  val viewUserOrderListEndpoint = tapirSecurity
    .tapirSecurityEndpoint(List(Roles.User)) // accessible only for users with role User
    .get // GET endpoint
    .description("Extracts orders for the user")
    .in("orders") // /orders uri
    .out(
      jsonBody[List[Order]] // defining response json format
    ...
    .serverLogic { user => _ => // endpoint logic definition
      ZioUtil.foldRunToFuture(
        OrderService.findOrdersForUser(user.id).provide(orderService)
    )
    }

  /** get order details endpoint definition */
  val viewUserOrderEndpoint = tapirSecurity
    .tapirSecurityEndpoint(List(Roles.User)) // accessible only for users with role User
    .get // GET endpoint
    .description("Retrieves order details.")
    .in(
      "orders" / path[String]("orderId")
        .description("Order's id to retrieve information")
    ) // /orders/:orderId uri
    .out(
      jsonBody[OrderWithRecords]
        .description("Contains order itself with it's entries") // set response json format
        .example(
          OrderWithRecords(
            Order(
              Util.generateUuid,
    ...
    .serverLogic { _ => orderId => // endpoint logic definition.
      ZioUtil.foldRunToFuture(
        OrderService.getOrderDetails(orderId).provide(orderService)
    )
    }

  /** Convenient way to assemble endpoints from the controller and then concat this route to main route. */
  val orderRoutes: List[ServerEndpoint[AkkaStreams with WebSockets, Future]] =
    List(createOrderEndpoint, viewUserOrderListEndpoint, viewUserOrderEndpoint)
}
