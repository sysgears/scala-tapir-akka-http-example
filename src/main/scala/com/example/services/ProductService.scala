  ...
  val live: ZLayer[ProductRepository, Nothing, services.ProductService.ProductService] = ZLayer {
    for {
      ...
      new Service {
         ...
         * @return metadata and extracted products.
         */
        override def extractPaginatedProducts(args: PaginatedEndpointArguments): ZIO[Any, ErrorInfo, PaginatedProductListViewResponse] = {
          ...
          (for {
            ...
            error =>
              logger.error(s"Intercepted error from extracting paginated products", error)
