...
import com.example.services.admin.AdminOrderService.{logger, AdminOrders}
...
object AdminOrderService extends LazyLogging {
  ...
  trait Service {
    ...
    def extractPaginatedOrders(
        args: PaginatedEndpointArguments
    ): ZIO[Any, ErrorInfo, AdminOrderViewResponse]

    def updateOrderStatus(
        args: AdminOrderStatusChangeArguments
    ): ZIO[Any, ErrorInfo, String]
    def deleteOrder(orderId: String): ZIO[Any, ErrorInfo, Unit]
  }

  def extractPaginatedOrders(
      args: PaginatedEndpointArguments
  ): ZIO[AdminOrders, ErrorInfo, AdminOrderViewResponse] =
    ZIO.serviceWithZIO[AdminOrders](_.extractPaginatedOrders(args))

  def updateOrderStatus(
      args: AdminOrderStatusChangeArguments
  ): ZIO[AdminOrders, ErrorInfo, String] =
    ZIO.serviceWithZIO[AdminOrders](_.updateOrderStatus(args))

  def deleteOrder(orderId: String): ZIO[AdminOrders, ErrorInfo, Unit] =
    ZIO.serviceWithZIO[AdminOrders](_.deleteOrder(orderId))

  val live: ZLayer[
    OrderProductRepository with UserRepository with ProductRepository with OrderRepository,
    Nothing,
    admin.AdminOrderService.AdminOrders
  ] = ZLayer {
    for {
      ...
      new AdminOrderService(orderDao, productDao, userDao, orderProductDao)
    }
  }
...
class AdminOrderService(
  ...
  override def extractPaginatedOrders(
      args: PaginatedEndpointArguments
  ): ZIO[Any, ErrorInfo, AdminOrderViewResponse] = {
    logger.trace(
      s"Started extracting paginated orders, page: ${args.page}, page size: ${args.pageSize}"
    )
    val offset = (args.page - 1) * args.pageSize
    val findPaginatedFuture =
      orderDao.findPaginated(args.pageSize, offset)
    ...
    ZioUtil.interceptSqlErrors(for {
      ...
      products <- productDao.findByIds(
        orderProducts.map(_.productId).distinct
      )
      users <- userDao.findByIds(orders.map(_.userId).distinct)
    } yield {
      val userOrders = orders.map { order =>
        val extendedOrderProducts =
          orderProducts.filter(_.orderId == order.id).map { orderProduct =>
            val product =
              products.find(_.id == orderProduct.productId)
            OrderRecord(product, orderProduct.quantity)
          }
        val user = users.find(_.id == order.userId)
        UserOrder(
          user.map(ShortUser(_)),
          OrderWithRecords(order, extendedOrderProducts)
        )
      }
      val pages =
        (orderCount.toDouble / args.pageSize.toDouble).ceil.toInt // calculating amount of available pages
      val metadata =
        PaginationMetadata(args.page, args.pageSize, pages, orderCount)
      logger.debug(
        s"Extracted paginated orders, pack size: ${userOrders.size}, response metadata: $metadata"
      )
  ...
  override def updateOrderStatus(
    ...
    ZioUtil
      ...
      .flatMap {
        case 0 =>
          ZIO.fail(NotFound(s"Order ${args.orderId} not found")) // if record wasn't updated
        case x if x > 0 => ZIO.succeed("Updated!") // success
        case _ =>
          ZIO.fail(
            InternalServerError("Unknown error, got less 0 result")
          ) // unexpected result
      }
  ...
  override def deleteOrder(orderId: String): ZIO[Any, ErrorInfo, Unit] = {
    ZioUtil.interceptSqlErrors(orderDao.remove(orderId)).map {
      case 0 =>
        ZIO.fail((NotFound(s"Order $orderId not found"))) // if record wasn't removed
