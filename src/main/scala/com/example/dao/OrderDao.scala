...
object OrderDao {
  ...
  trait Service {
    ...
    def updateStatus(orderId: String,
                     newStatus: String): ZIO[Any, SQLException, Long]

    /** Retrieves paginated orders. */
    def findPaginated(take: Int,
                      offset: Int): ZIO[Any, SQLException, List[Order]]
  ...
  val live
    : ZLayer[Quill.Postgres[SnakeCase], Nothing, dao.OrderDao.OrderRepository] =
    ZLayer {
      for {
