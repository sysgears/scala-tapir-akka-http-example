...
class ProductDao(context: Quill.Postgres[SnakeCase]) extends ProductRepository {
  import context._
  ...
  override def insert(product: Product): ZIO[Any, SQLException, Long] =
    run(products.insertValue(lift(product)))

  override def update(product: Product): ZIO[Any, SQLException, Long] =
    run(
      products.filter(_.id == lift(product.id)).updateValue(lift(product))
    )

  override def remove(productId: String): ZIO[Any, SQLException, Long] =
    run(products.filter(_.id == lift(productId)).delete)

  override def findAll(): ZIO[Any, SQLException, List[Product]] =
    run(products)
  ...
  override def countProducts(): ZIO[Any, SQLException, Long] =
    run(products.size)
  ...
  override def findByIds(
    ...
    run(
      products
        .filter(product => liftQuery(productIds).contains(product.id))
    )
}
