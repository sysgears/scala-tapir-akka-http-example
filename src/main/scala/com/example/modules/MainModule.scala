...
trait MainModule {
  ...
  val postgres: ZLayer[DataSource, Nothing, Quill.Postgres[SnakeCase.type]] = Quill.Postgres.fromNamingStrategy(SnakeCase)
  val dataSource: ZLayer[Any, Throwable, DataSource] = Quill.DataSource.fromPrefix("db.default")
