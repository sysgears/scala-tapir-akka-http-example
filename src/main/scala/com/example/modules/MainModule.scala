...
trait MainModule {
  ...
  lazy val userDao: ZLayer[Quill.Postgres[SnakeCase], Nothing, UserRepository] =
    UserDao.live
  lazy val orderDao
    : ZLayer[Quill.Postgres[SnakeCase], Nothing, OrderRepository] =
    OrderDao.live
  lazy val productDao
    : ZLayer[Quill.Postgres[SnakeCase], Nothing, ProductRepository] =
    ProductDao.live
  lazy val orderProductDao
    : ZLayer[Quill.Postgres[SnakeCase], Nothing, OrderProductRepository] =
    OrderProductDao.live
  lazy val jwtService: ZLayer[Config, Nothing, JwtService] = Jwt.live
  lazy val tapirAuth
    : ZLayer[UserRepository with JwtService, Nothing, TapirAuth] =
    TapirAuthentication.live
  lazy val authService
    : ZLayer[JwtService with UserRepository, Nothing, Authentication] =
    AuthService.live
  lazy val orderService
    : ZLayer[OrderProductRepository with ProductRepository with OrderRepository,
             Nothing,
             OrdersService] = OrderService.live
  lazy val productService: ZLayer[ProductRepository, Nothing, ProductService] =
    ProductService.live
  lazy val adminProductService
    : ZLayer[ProductRepository, Nothing, AdminProducts] =
  ...
  lazy val productServiceLayer: ZLayer[Any, Nothing, ProductService] = ZLayer
    ...
    .orDie
  lazy val adminProductServiceLayer
    : ZLayer[Any, Nothing, AdminProducts] = ZLayer
