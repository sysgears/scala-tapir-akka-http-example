...
object AuthServiceUnitTest extends ZIOSpecDefault {
  ...
  val testUser: User = User(Util.generateUuid, "test name", "+777777777", "test@example.com", "$2a$10$XMuxo.3xlVlGnySYCUOOIOl09n6olXw7aV2daE9Ief.Gd/js.Fq1O", "49050", "Dnipro", "test address", Roles.User, LocalDateTime.now())

  override def spec: Spec[TestEnvironment with Scope, Any] = suite("AuthService")(
    ...
    test("comparing correct user correctly") {
      ...
      //given
      when(userDao.findByEmail(any[String])).thenReturn(ZIO.succeed(Some(testUser)))
      when(jwtService.generateJwt(anyString())).thenReturn(ZIO.succeed("token"))
      val signInForm = SignInForm("test@example.com", "password")
      for {
        signInResult <- ZIO.serviceWithZIO[Authentication](_.signIn(signInForm)).provide(authService, ZLayer.succeed(userDao), ZLayer.succeed(jwtService))
      } yield {
    ...
    test("comparing incorrect user correctly") {
      ...
      assertZIO(ZIO.serviceWithZIO[Authentication](_.signIn(signInForm))
        .provide(authService, ZLayer.succeed(userDao), ZLayer.succeed(jwtService)).exit)(fails(isSubtype[ErrorInfo](anything)))
    },
    ...
    test("register new user correctly") {
      ...
      for {
        signUpResult <- ZIO.serviceWithZIO[Authentication](_.signUp(signUpForm)).provide(authService, ZLayer.succeed(userDao), ZLayer.succeed(jwtService))
      } yield {
        assertTrue(true)
      }
    },
    ...
    test("return conflict for registration new user") {
      ...
      // assert
      assertZIO(ZIO.serviceWithZIO[Authentication](_.signUp(signUpForm))
        .provide(authService, ZLayer.succeed(userDao), ZLayer.succeed(jwtService)).exit)(fails(isSubtype[ErrorInfo](anything)))
    },
